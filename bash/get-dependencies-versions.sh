...
logs_path="results/versions-logs.txt"
project_path=$1
id=$2
...
echo "Running safer dependencies versions for: $project_path, See outputs/$project_name/dependencies-versions.txt"
cd safer/src

start_time=$(date +%s)
# Use this line to create log files
mkdir -p ../../outputs/$project_name
PROJECT_ROOT_PATH="$project_root_path" npx tsx script.ts >../../outputs/$project_name/dependencies-versions.txt
...
# # Use this line to create log files
newer_versions=$(cat outputs/$project_name/dependencies-versions.txt | grep '^========= DEPENDENCIES NEWER VERSIONS ==========')
# # csv_line=$(echo "$output" | grep -A2 '^CSV:' | tail -n1)

if [ -z "$newer_versions" ]; then
  pretty_print red "Safer failed to execute in the project $project_name." >&2
  echo "[$id] $(date +"%T") Failure - $project_name." >>$logs_path
#   # gh repo delete safer-bot/$project_name --yes
else
  pretty_print green "Safer executed succesfully in the project $project_name.\nSee outputs/$project_name/dependencies-versions.txt and $dataset_path"
  echo "[$id] $(date +"%T") Success - Got dependencies versions of $project_name." >>$logs_path
fi
